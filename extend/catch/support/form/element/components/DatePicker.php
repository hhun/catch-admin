 ...
 * @method $this unlinkPanels(bool $unlinkPanels = true) 在范围选择器里取消两个日期面板之间的联动, 默认值: false
 ...
 * @method $this validateEvent(bool $validateEvent = true) 输入时是否触发表单的校验, 默认值: true
 * @method $this timeArrowControl(bool $timeArrowControl = true) 是否使用箭头进行时间选择, 默认值: false
 *
 *
 */
class DatePicker extends FormComponent
{
    ...
    public function createValidate()
    {
        if ($this->isRange() || $this->isMultiple()) {
            ...
            return ValidateFactory::validateDate();
        }
    }
    ...
    public function required(string $message = null): self
    {
        ...
        if ($this->isRange()) {
            $dateRequired = ValidateFactory::validateStr()->message($message)->required();
            $validate->fields([
                '0' => $dateRequired,
                '1' => $dateRequired
            ]);

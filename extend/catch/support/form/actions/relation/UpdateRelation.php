<?php
namespace catch\support\form\actions\relation;

use catch\exceptions\FailedException;

trait UpdateRelation
{
    /**
        ...
        foreach ($relations as $relation) {
            if (isset($this->form[$relation])) {
                if (method_exists($this->model, $relation)) {
                    $this->model->{$relation}()->sync($this->form[$relation]);
                } else {
        ...
        foreach ($relations as $relation) {
            if (isset($this->form[$relation])) {
                if (method_exists($this->model, $relation)) {
                    ...
                    $this->model->{$relation}()->save(new $relateModel($this->form[$relation]));
                } else {
